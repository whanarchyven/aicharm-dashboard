        ...
        <InfoTab className={'px-4'} icon={'brain'} title={'Text analytics'}>
            <div className={'h-full px-4 relative py-4 pb-0 overflow-y-scroll items-end'}>
                {messages.map((item:any,counter)=>{
                    return(
                        <div key={counter} className={'bg-[#A34EDF] my-3 bg-opacity-20 gap-0 rounded-lg grid items-start p-3 grid-cols-12 w-full'}>
                ...
                <div className={'bg-[#A34EDF] w-full bottom-0 left-0 h-16 flex items-center gap-3 rounded-xl px-4 py-2 sticky'}>
                    <div className={'w-full h-full rounded-xl border-2 border-white'}>
                        <textarea onChange={(event)=>{setUserMessage(event.target.value)}} className={classList('w-full h-full bg-transparent outline-none p-2',isLoading?'opacity-50':'opacity-100')}></textarea>
                    </div>
                    {isLoading?<img src={'/loading.svg'} className={'w-12 animate-spin p-2 aspect-square'}/>:<div className={'w-12 bg-white p-2 rounded-full flex items-center justify-center cursor-pointer aspect-square'} onClick={()=>{
                        setIsloading(true);
                        axios.post(url,{
                            text:userMessage,
